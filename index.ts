...
function toFunctional<T extends Function>(func:T):Function{
    const numOfArg = func.length;
    function createSubFunction(curriedArg:unknown[]){
        return function(this:unknown){
            const newCurriedArguments = curriedArg.concat(Array.from(arguments));
            if(newCurriedArguments.length > numOfArg){
 ...
 * @return {* | Function}
 */
export const reduce = toFunctional(<I,O>(reducer:(acc:O, item:I)=>O, initialValue:O, input:I[])=> input.reduce(reducer, initialValue)) as ReduceFunc;
